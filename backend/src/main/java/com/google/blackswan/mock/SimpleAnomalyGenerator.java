...
public class SimpleAnomalyGenerator implements AnomalyGenerator {
  ...
  private Anomaly createAnomalyFromDataPoint(Timestamp time, int value) {
    ...
    int start = (indexOfAnomaly - NUM_POINTS >= 0) ? indexOfAnomaly - NUM_POINTS : 0;
    int end = (indexOfAnomaly + NUM_POINTS < data.size()) ? indexOfAnomaly + NUM_POINTS : data.size() - 1;

    Map<Timestamp, MetricValue> dataPoints = new LinkedHashMap<>();
